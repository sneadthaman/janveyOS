...
async function main() {
  ...
  const res = await fetch(`${baseUrl}/api/tools/pricing-lookup`, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      ...(secret ? { "x-agent-secret": secret } : {})
    },
    body: JSON.stringify({ sku: "DIV 95892221", customer: "CHS", requested_by: "test-agent", source: "openclaw" })
  });
