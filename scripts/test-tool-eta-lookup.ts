...
async function main() {
  ...
  const res = await fetch(`${baseUrl}/api/tools/eta-lookup`, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      ...(secret ? { "x-agent-secret": secret } : {})
    },
    body: JSON.stringify({ sku: "DIV 95892221", customer: "CHS", sales_order: "SO12345", requested_by: "test-agent", source: "openclaw" })
  });
